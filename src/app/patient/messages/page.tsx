            ...
            <section className="w-80 bg-surface-container flex flex-col border-r border-outline-variant/10">
                <div className="p-6 pb-4">
                    <div className="flex items-center gap-4 mb-6">
                        <Link href="/patient/home" className="hover:bg-surface-container-highest p-1 rounded-full"><span className="material-symbols-outlined">arrow_back</span></Link>
                        <h1 className="font-headline text-2xl font-extrabold text-on-surface">Messages</h1>
                    </div>
                    <button disabled={creating} onClick={handleStartChat} className="w-full flex items-center justify-center gap-2 py-3 bg-primary text-on-primary rounded-xl font-bold hover:opacity-90 transition-opacity outline-none disabled:opacity-50">
                ...
                <div className="flex-1 overflow-y-auto px-3 flex flex-col gap-1 pb-4 no-scrollbar mt-4">
                    {threadsLoading ? (
                        ...
                        <div className="text-center text-outline text-sm p-4 mt-6">No active conversations. Start a chat with a doctor.</div>
                    ) : (
                        threads.map(thread => {
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline mb-1">
                                                <h3 className="font-headline font-bold text-on-surface truncate text-sm">Provider</h3>
                    ...
                    <>
                        <header className="h-20 bg-surface/70 backdrop-blur-md border-b border-outline-variant/10 px-8 flex items-center justify-between z-10">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h2 className="font-headline text-lg font-extrabold text-on-surface leading-tight">Care Provider</h2>
                        ...
                        <footer className="p-6 bg-surface/80 backdrop-blur-lg border-t border-outline-variant/10">
                            <div className="max-w-4xl mx-auto flex items-end gap-3">
                                <div className="flex-1 bg-surface-container-low rounded-2xl flex flex-col p-2 border border-transparent transition-all focus-within:bg-white focus-within:shadow-md">
                                    <textarea
                                        value={inputMsg}
                                        onChange={e => setInputMsg(e.target.value)}
                                        onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                                        className="w-full bg-transparent border-none focus:ring-0 text-sm p-3 resize-none max-h-32 outline-none"
                                    ...
                                    <div className="flex items-center justify-end px-3 pb-1 pt-2 border-t border-outline-variant/5">
                                        <button onClick={handleSend} disabled={!inputMsg.trim()} className="bg-primary text-on-primary p-2 w-10 h-10 rounded-xl hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center outline-none">
